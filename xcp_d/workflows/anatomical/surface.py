    ...
    for hemi in ['L', 'R']:
        ...
        workflow.connect([
            ...
            (update_xfm_wf, apply_transforms_wf, [
                ...
                ('outputnode.fwd_order', 'inputnode.fwd_order'),
                ('outputnode.rvs_order', 'inputnode.rvs_order'),
            ]),
...
def init_ants_xfm_to_fsl_wf(mem_gb, name='ants_xfm_to_fsl_wf'):
    ...
    workflow.connect([
        ...
        (disassemble_h5, outputnode, [('order', 'fwd_order')]),
    ])  # fmt:skip

    # TODO: Invert the order of the individual transforms here
    ...
    workflow.connect([
        ...
        (disassemble_h5_inv, outputnode, [('order', 'rvs_order')]),
    ])  # fmt:skip
...
def init_warp_one_hemisphere_wf(
    ...
    # XXX: We may need to skip this step for fsLR-32k surfaces generated by fMRIPrep and Nibabies.
...
class ApplyTransformsWorkflow(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if self.inputs.fwd_order != self.inputs.rvs_order[::-1]:
            # The anat-to-template should have AWAW order and the template-to-anat should have
            # WAWA order.
            raise ValueError('The forward and reverse order of transforms must match.')
        ...
        inputnode.inputs.merged_inv_warpfields = self.inputs.merged_inv_warpfields[::-1]

...
def get_transformfilex(bold_file, mni_to_t1w, t1w_to_native):
    ...
    MNI6 = str(get_template(template='MNI152NLin2009cAsym',
               mode='image', suffix='xfm', extension='.h5'))
...
def get_transformfile(bold_file, mni_to_t1w, t1w_to_native):
    ...
    fMNI6 = str(get_template(template='MNI152NLin2009cAsym',
                mode='image', suffix='xfm', extension='.h5'))
    FSL2MNI9 = pkgrf('xcp_d', 'data/transform/FSL2MNI9Composite.h5')
...
def get_customfile(custom_conf, bold_file):
    if custom_conf is not None:
        ...
        custom_file = os.path.abspath(str(custom_conf) + '/' +
                                      file_base + '-custom_timeseries.tsv')
    else:
        custom_file = None
    return custom_file

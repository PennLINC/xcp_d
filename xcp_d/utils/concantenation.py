    ...
    if not cifti:
        for s in subjlist:
            ...
            if sed:
                ...
                for kses in ses:
                    concatenate_nifti(subid=_prefix(s), fmridir=fmridir,
                                      outputdir=outputdir, ses=kses, work_dir=work_dir)
            else:
                ses = None
                concatenate_nifti(subid=_prefix(s), fmridir=fmridir,
        ...
        for s in subjlist:
            ...
            if sed:
                ses = list(set([_getsesid(j) for j in sed]))
                for kses in ses:
                    concatenate_cifti(subid=_prefix(s), fmridir=fmridir,
                                      outputdir=outputdir, ses=kses, work_dir=work_dir)
            else:
                concatenate_cifti(subid=_prefix(s), fmridir=fmridir,
                                  outputdir=outputdir, work_dir=work_dir)
...
def make_DCAN_DF(fds_files, name):
    ...
    for thresh in np.linspace(0, 1, 101):
        ...
        dcan.create_dataset(
            "/dcan_motion/fd_{0}/binary_mask".format(thresh),
            data=(fd > thresh).astype(int), dtype='float')
        dcan.create_dataset(
            "/dcan_motion/fd_{0}/threshold".format(thresh),
            data=thresh, dtype='float')
        dcan.create_dataset(
            "/dcan_motion/fd_{0}/total_frame_count".format(thresh), data=len(fd), dtype='float')
        dcan.create_dataset(
...
def concatenate_nifti(subid, fmridir, outputdir, ses=None, work_dir=None):
    ...
    datafile = ['_atlas-Glasser_desc-timeseries_bold.tsv',
    ...
    for task in tasklist:
        ...
        if len(resbold) > 1:
            ...
            for j in datafile:
                ...
                filex = natsorted(
                    glob.glob(res.split('run-')[0] + '*run*' + resid.partition('_desc')[0] + j))

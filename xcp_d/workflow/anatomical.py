...
def init_anatomical_wf(
    ...
    mnitemplate = str(
        get_template(template='MNI152NLin6Asym', resolution=2,
                     suffix='T1w')[-1])
    ...
    if input_type == 'dcan' or input_type == 'hcp':
        ds_t1wmni_wf = pe.Node(DerivativesDataSink(base_directory=output_dir,
        ...
        ds_t1wseg_wf = pe.Node(DerivativesDataSink(base_directory=output_dir,
                                                   space='MNI152NLin6Asym',
                                                   suffix='dseg',
                                                   extension='.nii.gz'),
                               name='ds_t1wseg_wf',
        ...
        L_midthick_surf = fnmatch.filter(
            all_files,
            '*sub-*' + subject_id + '*hemi-L_midthickness.surf.gii')[0]
        R_midthick_surf = fnmatch.filter(
            all_files,
            '*sub-*' + subject_id + '*hemi-R_midthickness.surf.gii')[0]
        ...
        ribbon = fnmatch.filter(all_files, '*sub-*' + subject_id +
                                '*desc-ribbon.nii.gz')[0]
        ...
        surf = [
            L_inflated_surf, R_inflated_surf, L_midthick_surf, R_midthick_surf,
            L_pial_surf, R_pial_surf, L_wm_surf, R_wm_surf
        ]
        ...
        brainspritex_wf = pe.Node(BrainPlotx(),
                                  name='brainsprite',
                                  mem_gb=mem_gb,
                                  n_procs=omp_nthreads)

        from ..utils import ContrastEnhancement
        enhancet1w_wf = pe.Node(ContrastEnhancement(),
                                name='enhancet1w',
                                mem_gb=mem_gb,
                                n_procs=omp_nthreads)

        ds_brainspriteplot_wf = pe.Node(DerivativesDataSink(
        ...
        t1w_transform_wf = pe.Node(ApplyTransformsx(
            num_threads=2,
        ...
        seg_transform_wf = pe.Node(ApplyTransformsx(
            num_threads=2,
            reference_image=mnitemplate,
            transforms=[str(t1w_to_mni), str(MNI92FSL)],
        ...
        p = Path(fmri_dir)
        import glob as glob
        freesufer_paths = glob.glob(
            str(p.parent) + '/freesurfer*')  # for fmriprep and nibabies
        if len(freesufer_paths) == 0:
            freesufer_paths = glob.glob(str(p) +
                                        '/sourcedata/*freesurfer*')  # nibabies

        if len(freesufer_paths) > 0 and 'freesurfer' in os.path.basename(
                freesufer_paths[0]):
        ...
        if freesufer_path is not None and os.path.isdir(freesufer_path):
            ...
            L_inflated_surf = fnmatch.filter(
                all_files,
                '*sub-*' + subject_id + '*hemi-L_inflated.surf.gii')[0]
            R_inflated_surf = fnmatch.filter(
                all_files,
                '*sub-*' + subject_id + '*hemi-R_inflated.surf.gii')[0]
            L_midthick_surf = fnmatch.filter(
                all_files,
                '*sub-*' + subject_id + '*hemi-L_midthickness.surf.gii')[0]
            R_midthick_surf = fnmatch.filter(
            ...
            L_wm_surf = fnmatch.filter(
                all_files,
                '*sub-*' + subject_id + '*hemi-L_smoothwm.surf.gii')[0]
            R_wm_surf = fnmatch.filter(
                all_files,
                '*sub-*' + subject_id + '*hemi-R_smoothwm.surf.gii')[0]
            ...
            left_sphere_raw = str(
                freesufer_path) + '/' + subid + '/surf/lh.sphere'  # MB
            right_sphere_raw = str(
                freesufer_path) + '/' + subid + '/surf/rh.sphere'  # MB
            ...
            left_sphere_raw_mris = pe.Node(MRIsConvert(
                out_datatype='gii', in_file=left_sphere_raw),
                name='left_sphere_raw_mris',
                mem_gb=mem_gb,
                n_procs=omp_nthreads)  # MB
            right_sphere_raw_mris = pe.Node(MRIsConvert(
                out_datatype='gii', in_file=right_sphere_raw),
            ...
            # anat/sub-MSC01_ses-3TAME01_from-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5
            # T1w_to_MNI152Lin6Asym

            h5_file = fnmatch.filter(
                all_files, '*sub-*' + subject_id + '*from'
                '-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5')[0]  # MB
            disassemble_h5 = pe.Node(CompositeTransformUtil(
                process='disassemble',
                in_file=h5_file,
            ...
            convert_ants_transform = pe.Node(
                ...
                name="convert_ants_transform")  # MB
            ...
            # 00_T1w_to_MNI152Lin6Asym_AffineTransform.txt >
            # 00_T1w_to_MNI152Lin6Asym_AffineTransform.txt
            change_xfm_type = pe.Node(ChangeXfmType(),
                                      name="change_xfm_type")  # MB

            # convert affine xfm to "world" so it
            # works with -surface-apply-affine
            # wb_command -convert-affine -from-itk
            # 00_T1w_to_MNI152Lin6Asym_AffineTransform.txt
            # -to-world 00_T1w_to_MNI152Lin6Asym_AffineTransform_world.nii.gz
            convert_xfm2world = pe.Node(ConvertAffine(fromwhat='itk',
                                                      towhat='world'),
                                        name="convert_xfm2world")  # MB
            ...
            fs_LR2fs_R = pkgrf(
                'xcp_d',
                'data/standard_mesh_atlases/fs_R/fs_R-to-fs_LR_fsaverage'
            ...
            surface_apply_affine_lh_pial = pe.Node(
                ApplyAffine(in_file=L_pial_surf),
                name='surface_apply_affine_lh_pial')  # MB
            surface_apply_affine_rh_pial = pe.Node(
                ApplyAffine(in_file=R_pial_surf),
                name='surface_apply_affine_rh_pial')  # MB
            surface_apply_affine_lh_wm = pe.Node(
                ApplyAffine(in_file=L_wm_surf),
                name='surface_apply_affine_lh_wm')  # MB
            surface_apply_affine_rh_wm = pe.Node(
                ApplyAffine(in_file=R_wm_surf),
                name='surface_apply_affine_rh_wm')  # MB
            ...
            apply_warpfield_lh_wm = pe.Node(ApplyWarpfield(),
                                            name='apply_warpfield_lh_wm')  # MB
            apply_warpfield_rh_wm = pe.Node(ApplyWarpfield(),
                                            name='apply_warpfield_rh_wm')
            # MB
            ...
            left_wm_surf_wf = pe.Node(CiftiSurfaceResample(
                ...
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            left_pial_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=left_sphere_fsLR,
                metric=' BARYCENTRIC ',
                in_file=L_pial_surf),
                name="left_pial_surf",
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            left_midthick_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=left_sphere_fsLR,
                metric=' BARYCENTRIC ',
                ...
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            left_inf_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=left_sphere_fsLR,
                metric=' BARYCENTRIC ',
            ...
            right_wm_surf_wf = pe.Node(CiftiSurfaceResample(
                ...
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            right_pial_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=right_sphere_fsLR,
                metric=' BARYCENTRIC ',
                in_file=R_pial_surf),
                ...
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            right_midthick_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=right_sphere_fsLR,
                metric=' BARYCENTRIC ',
                ...
                mem_gb=mem_gb,
                n_procs=omp_nthreads)
            right_inf_surf_wf = pe.Node(CiftiSurfaceResample(
                new_sphere=right_sphere_fsLR,
                metric=' BARYCENTRIC ',
            ...
            # write report node
            ds_wmLsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_wmRsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_pialLsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
                ...
                run_without_submitting=True,
                mem_gb=2)
            ds_pialRsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_infLsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_infRsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_midLsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            ds_midRsurf_wf = pe.Node(DerivativesDataSink(
                base_directory=output_dir,
                dismiss_entities=['desc'],
                space='fsLR',
                density='32k',
            ...
            workflow.connect([(disassemble_h5, convert_ants_transform,
                               [('affine_transform', 'in_transform')]),
                              (convert_ants_transform, change_xfm_type,
                               [('out_transform', 'in_transform')]),
                              (change_xfm_type, convert_xfm2world,
                               [('out_transform', 'in_file')])])

            workflow.connect([
                (convert_xfm2world, surface_apply_affine_lh_pial,
                 [('out_file', 'affine')]),
                (surface_apply_affine_lh_pial, apply_warpfield_lh_pial,
                 [('out_file', 'in_file')]),
                (disassemble_h5, apply_warpfield_lh_pial,
                 [('displacement_field', 'warpfield')]),
                (apply_warpfield_lh_pial, left_pial_surf_wf, [('out_file',
                                                               'in_file')]),
                (left_sphere_raw_mris, left_pial_surf_wf,
            ...
            workflow.connect([
                (convert_xfm2world, surface_apply_affine_rh_pial,
                 [('out_file', 'affine')]),
                (surface_apply_affine_rh_pial, apply_warpfield_rh_pial,
                 [('out_file', 'in_file')]),
                (disassemble_h5, apply_warpfield_rh_pial,
                 [('displacement_field', 'warpfield')]),
                (apply_warpfield_rh_pial, right_pial_surf_wf, [('out_file',
                                                                'in_file')]),
                (right_sphere_raw_mris, right_pial_surf_wf,
                 [('converted', 'current_sphere')]),
            ...
            workflow.connect([
                (convert_xfm2world, surface_apply_affine_lh_wm, [('out_file',
                                                                  'affine')]),
                (surface_apply_affine_lh_wm, apply_warpfield_lh_wm,
                 [('out_file', 'in_file')]),
                (disassemble_h5, apply_warpfield_lh_wm, [('displacement_field',
                                                          'warpfield')]),
                (apply_warpfield_lh_wm, left_wm_surf_wf, [('out_file',
                                                           'in_file')]),
                (left_sphere_raw_mris, left_wm_surf_wf, [('converted',
                                                          'current_sphere')]),
                (left_wm_surf_wf, ds_wmLsurf_wf, [('out_file', 'in_file')]),
            ])

            workflow.connect([
                (convert_xfm2world, surface_apply_affine_rh_wm, [('out_file',
                                                                  'affine')]),
                (surface_apply_affine_rh_wm, apply_warpfield_rh_wm,
                 [('out_file', 'in_file')]),
                (disassemble_h5, apply_warpfield_rh_wm, [('displacement_field',
                                                          'warpfield')]),
                (apply_warpfield_rh_wm, right_wm_surf_wf, [('out_file',
                                                            'in_file')]),
                (right_sphere_raw_mris, right_wm_surf_wf,
                 [('converted', 'current_sphere')]),
            ...
            workflow.connect([
                ...
                (brainspritex_wf, ds_brainspriteplot_wf, [('out_html',
                                                           'in_file')]),
                (inputnode, ds_brainspriteplot_wf, [('t1w', 'source_file')]),
            ])
            ...
            enhancet1w_wf = pe.Node(ContrastEnhancement(),
                                    ...
                                    mem_gb=mem_gb,
                                    n_procs=omp_nthreads)
            ribbon2statmap_wf = pe.Node(RibbontoStatmap(),
                                        name='ribbon2statmap',
                                        mem_gb=mem_gb,
                                        n_procs=omp_nthreads)
            brainspritex_wf = pe.Node(BrainPlotx(),

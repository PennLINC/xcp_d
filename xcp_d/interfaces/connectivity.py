...
class _GroupSurfacesInputSpec(BaseInterfaceInputSpec):
    lh_sulcal_depth = traits.Either(None, File(exists=True))
    rh_sulcal_depth = traits.Either(None, File(exists=True))
    lh_sulcal_curv = traits.Either(None, File(exists=True))
    rh_sulcal_curv = traits.Either(None, File(exists=True))
    lh_cortical_thickness = traits.Either(None, File(exists=True))
    rh_cortical_thickness = traits.Either(None, File(exists=True))


class _GroupSurfacesOutputSpec(TraitedSpec):
    lh_files = traits.List(
        traits.File(exists=True),
        desc="Filtered list of left-hemisphere files.",
    )
    rh_files = traits.List(
        traits.File(exists=True),
        desc="Filtered list of right-hemisphere files.",
    )
    descriptions = traits.List(traits.Str, desc="The 'desc' field for each file.")


class GroupSurfaces(SimpleInterface):
    """Group surface files and filter out missing files."""

    input_spec = _GroupSurfacesInputSpec
    output_spec = _GroupSurfacesOutputSpec
    ...
    def _run_interface(self, runtime):
        ...
        self._results["lh_files"], self._results["rh_files"] = [], []
        self._results["descriptions"] = []
        for surface, desc in SURF_DESCS.items():
            lh_surf = getattr(self.inputs, f"lh_{surface}")
            rh_surf = getattr(self.inputs, f"rh_{surface}")
            if isinstance(lh_surf, str) and isinstance(rh_surf, str):
                self._results["lh_files"].append(lh_surf)
                self._results["rh_files"].append(rh_surf)
                self._results["descriptions"].append(desc)

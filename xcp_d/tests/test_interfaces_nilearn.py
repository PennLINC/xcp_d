...
def test_nilearn_smooth(pnc_data, tmp_path_factory):
...
def test_nilearn_resampletoimage(pnc_data, tmp_path_factory):
    """Test xcp_d.interfaces.nilearn.ResampleToImage."""
    tmpdir = tmp_path_factory.mktemp("test_nilearn_meanimage")

    source_file = pnc_data["boldref_t1w"]
    target_file = pnc_data["t1w"]
...
def test_nilearn_denoisenifti(pnc_data, tmp_path_factory):
    """Test xcp_d.interfaces.nilearn.DenoiseNifti."""
    tmpdir = tmp_path_factory.mktemp("test_nilearn_denoisenifti")

    preprocessed_bold = pnc_data["nifti_file"]
    mask = pnc_data["brain_mask_file"]
...
def test_nilearn_denoisecifti(pnc_data, tmp_path_factory):

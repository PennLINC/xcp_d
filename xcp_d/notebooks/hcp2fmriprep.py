...
All subjects from the S1200 HCP-YA were analyzed. For each Task
("REST1","REST2","WM","MOTOR","GAMBLING",
"EMOTION","LANGUAGE","SOCIAL") and Encoding Direction ("LR","RL"),
we analyzed the session if the following
files were present:
...
To ensure this process was identical, we generated these time series for the rfMRI sessions
and compared them to the HCP distributed timeseries, ensuring they are identical.
These files were then formatted into fMRIprep outputs by renaming the files,
creating the regression json, and creating dummy transforms. These inputs were then analyzed by
xcp_abcd with the following command:
singularity run --cleanenv -B ${PWD} ~/xcp_hcp/xcp-abcd-latest.sif
/$SUBJECT/fmriprepdir/ ~/xcp_hcp/xcp_results/ participant --cifti --despike
--lower-bpf 0.01 --upper-bpf 0.08 --participant_label sub-$SUBJECT -p 36P -f 100
--omp-nthreads 4 --nthreads 4
...
def remove(subid):
    for f in glob.glob(
            ...
            % (subid)):
        if ('Schaefer417_den-91k_den-91k_bold.pconn.nii' not in f and
            'Schaefer417_den-91k_den-91k_bold.ptseries.nii' not in f and
           'qc_den-91k_bold.tsv' not in f):
            os.system('rm -f %s' % (f))
...
if function == 'sge':
    ...
    for sub in glob.glob('/cbica/projects/HCP_Data_Releases/HCP_1200/**'):
        ...
        if sub == 'ToSync':
            continue
        os.system(
            'qsub -l h_vmem={0}G,s_vmem={0}G -N p{1} -pe threaded 4 -V -j y -b'
            'y -o ~/sge/ -e ~/sge/ python /cbica/home/bertolem/xcp_hcp/hcp2fmriprep.py run {1}'
            .format(36, sub))
...
if function == 'run':
    ...
    for j in tasklist:
        ...
        hcp_mask = '/{0}/{1}//MNINonLinear/Results/{2}/brainmask_fs.2.nii.gz'.format(
            hcp_dir, subid, j)
        prep_mask = funcdir+'/sub-'+subid+'_task-'+taskname+'_acq-' + \
            acqname + '_space-MNI152NLin6Asym_desc-brain_mask.nii.gz'
        copyfile(hcp_mask, prep_mask)

        # create/copy  cifti
        # to get TR  and just sample
        niftip = '{0}/{1}/MNINonLinear/Results/{2}/{2}.nii.gz'.format(hcp_dir, subid, j)
...
# qsub -l h_vmem=128G,s_vmem=128G -N zipit -V
# -j y -b y -o ~/sge/ -e ~/sge/ python
# /cbica/home/bertolem/xcp_hcp/hcp2fmriprep.py zipit None
...
if function == 'zipit':
    ...
    for parcel in parcels:
        for matrix in df.iterrows():
            ...
            'sub-{0}_task-{1}_acq-{3}_space-fsLR_atlas-{2}_den-91k_den-91k_bold'
            '.pconn.nii'.format(matrix['sub'], matrix['task'], parcel, matrix['acq'])
            ...
            fname = '/cbica/home/bertolem/xcp_hcp/xcp_results/xcp_abcd/sub-{0}/'
            'func/sub-{0}_task-{1}_acq-{3}_space-fsLR_atlas-{2}_den-91k_den-91k_'
            'bold.ptseries.nii'.format(
...
def make_hcp():
    ...
    for sub in glob.glob('/cbica/projects/HCP_Data_Releases/HCP_1200/**'):
        ...
        if subject == 'ToSync':
            continue
        files = glob.glob(
            '/cbica/projects/HCP_Data_Releases/HCP_1200/{0}/MNINonLinear'
            '/Results/*rfMRI_REST*/rfMRI_**_Atlas_MSMAll_hp2000_clean.dtseries.nii'
            .format(subject))
        m = []
        for f in files:
            run = f.split('/')[8]
            surf = nb.load(f)
            data = surf.get_fdata()
            command = 'wb_command -cifti-stats {0} -reduce MEAN >>'
            '/cbica/home/bertolem/xcp_hcp/WBS.txt'.format(f)
            os.system(command)
            ...
            p = '/cbica/home/bertolem/yeo_parcels/Schaefer2018_400Parcels'
            '_17Networks_order.dlabel.nii'

...
1. Summary reports: There are two summary reports- executive summary per session and  participant summary::
       
       xcp_abcd/sub-${sub-id}_ses-${ses-id}_executive_summary.html
       xcp_abcd/sub-${sub-id}.html

2. Anatomical outputs: Anatomical outputs conists of anatomical preprocessed T1w/T2w  and segmentations images in MNI spaces::
   ...
   If there are gifti files in fMRIPrep output, the giftifiles are resmaple to standard sapce::

...
However, we strongly recommend using :ref:`Installation:Container Technologies`. TEST. Here, the command-line will be composed of a preamble to configure the container execution followed by the ``xcp_d`` command-line options as if you were running it on a *bare-metal* installation.

...
def make_hcp():
	...
	for sub in glob.glob('/cbica/projects/HCP_Data_Releases/HCP_1200/**'):
		...
		if subject == 'ToSync':
			continue
		files = glob.glob('/cbica/projects/HCP_Data_Releases/HCP_1200/{0}/MNINonLinear/Results/*rfMRI_REST*/rfMRI_**_Atlas_MSMAll_hp2000_clean.dtseries.nii'.format(subject))
		m = []
		for f in files:
			run = f.split('/')[8]
			surf = nb.load(f)
			...
			new_img = nb.Cifti2Image(d, header=surf.header,nifti_header=surf.nifti_header)
			cifti_out = "/cbica/home/bertolem/xcp_hcp/{0}_{1}.dtseries.nii".format(subject,run)
			new_img.to_filename(cifti_out)
			p= '/cbica/home/bertolem/yeo_parcels/Schaefer2018_400Parcels_17Networks_order.dlabel.nii'
			out = '/cbica/home/bertolem/xcp_hcp/{0}_{1}_{2}.ptseries.nii'.format(subject,run,p.split('/')[-1].split('.')[0])
			cmd =  'wb_command -cifti-parcellate {0} {1} COLUMN {2}'.format(cifti_out,p,out)
			os.system(cmd)
			os.system('rm -f {0}'.format(cifti_out))
			ts = nb.load(out).get_fdata()
			os.system('rm -f {0}'.format(out))
			m.append(np.arctanh(np.corrcoef(ts.T)))
		np.save('/cbica/home/bertolem/xcp_hcp/rest_fixgsr/{0}.npy'.format(subject),np.nanmean(m,axis=0))
...
def compare():
	...
	for m in hcp_matrices:
		...
		hcp_m = np.load(m)
		xcp = []
		files = glob.glob('/cbica/home/bertolem/xcp_hcp/xcp_results/xcp_abcd/sub-{0}/func/*REST*pconn*'.format(s))

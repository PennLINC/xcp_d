...
def hcpfmriprepx(hcp_dir,out_dir,subid):
    ...
    hcpfiles = [tw1,segm,ribbon, brainmask,tw1,tw1,midL,midR,pialL,pialR,whiteL,whiteR,infL,infR]
    ...
    for k in tasklistx:
        ...
        regressors.to_csv(funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_desc-confounds_timeseries.tsv',index=False,sep= '\t')
        regressors.to_json(funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_desc-confounds_timeseries.json')
    
        # functional files
        hcp_ref = k + '/' + filenamex + '_SBRef.nii.gz'
        prep_ref = funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_space-MNI152NLin6Asym_boldref.nii.gz'

        # create/copy  cifti
        ciftip = k + '/'+ filenamex +'_Atlas_MSMAll.dtseries.nii'
        ciftib = funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_space-fsLR_den-91k_bold.dtseries.nii'
        ...
        boldjson = funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_space-MNI152NLin6Asym_desc-preproc_bold.json'
        ciftijson = funcdir+'/sub-'+subid+'_task-'+idx[1]+'_acq-'+idx[2]+'_space-fsLR_den-91k_bold.dtseries.json'

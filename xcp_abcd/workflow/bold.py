...
Residual timeseries from this regression were then band-pass filtered to retain signals within the  {highpass}-{lowpass} Hz frequency band. 
 """.format(regressors=stringforparams(params=params),sclver=sklearn.__version__,
    ...
    fcon_ts_wf = init_fcon_ts_wf(mem_gb=4,mni_to_t1w=mni_to_t1w,
    ...
    censorscrub_wf = init_censoring_wf(mem_gb=4,TR=TR,custom_conf=custom_conf,head_radius=head_radius,
                contigvol=contigvol,dummytime=dummytime,fd_thresh=fd_thresh,name='censoring',omp_nthreads=omp_nthreads)
    
    resdsmoothing_wf = init_resd_smoohthing(mem_gb=4,smoothing=smoothing,cifti=False,
    ...
    executivesummary_wf =init_execsummary_wf(tr=TR,bold_file=bold_file,layout=layout,mem_gb=2,
    ...
    resample_parc = pe.Node(ApplyTransforms(
        ...
        name='resample_parc',n_procs=omp_nthreads)
    ...
    resample_bold2T1w = pe.Node(ApplyTransforms(
        dimension=3,
         ...
         name='bold2t1_trans',n_procs=omp_nthreads)
    ...
    resample_bold2MNI = pe.Node(ApplyTransforms(
        dimension=3,
         ...
         name='bold2mni_trans',n_procs=omp_nthreads)
    ...
    qcreport = pe.Node(computeqcplot(TR=TR,bold_file=bold_file,dummytime=dummytime,t1w_mask=t1w_mask,
                       template_mask = str(get_template('MNI152NLin2009cAsym', resolution=2, desc='brain',
                        suffix='mask', extension=['.nii', '.nii.gz'])),
                       head_radius=head_radius), name="qc_report",n_procs=omp_nthreads)

...
def test_lp_notch():
    ...
    assert (
        np.mean(np.correlate(notch_data.squeeze(), notch_data_comparator.squeeze()))
        > 0.95
    )
    # Assert a high correlation
...
def test_butterworth():
    ...
    diff = np.sum(
        abs(np.array(butterworth_data) - np.array(butterworth_data_comparator))
    )
    ...
    assert (
        np.mean(
            np.correlate(
                butterworth_data.squeeze(), butterworth_data_comparator.squeeze()
            )
        )
        > 0.95
    )

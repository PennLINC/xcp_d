...
run_pytest_cmd () {
  ...
  if [[ "${CIRCLECI}" = "true" ]]; then
    ...
    PYTEST_RUN="pytest -m 'nibabies' --data_dir=${data_dir}/data --output_dir=${output_dir} --working_dir=${workdir} tests/"
  else
    patch_mount=""
    ...
    PYTEST_RUN+='-m nibabies --data_dir=/bids-input/data --output_dir=/out --working_dir=/work /rundir/'

...
run_pytest_cmd () {
  ...
  if [[ "${CIRCLECI}" = "true" ]]; then
    ...
    PYTEST_RUN="pytest --data_dir=${data_dir}/data --output_dir=${output_dir} --working_dir=${workdir} tests/ -k test_cifti_conn"
  else
    patch_mount=""
    ...
    PYTEST_RUN+='--data_dir=/bids-input/data --output_dir=/out --working_dir=/work /usr/local/miniconda/lib/python3.8/site-packages/xcp_d -k test_cifti_parcellation_resampling'
